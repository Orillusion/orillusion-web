...
Although the engine already includes some built-in post-processing effects, it does not cover all requirements. Therefore, you can create custom post-processing objects to achieve the post-processing effects your project needs.
This chapter uses the Gaussian blur post-processing effect in a `ComputeShader` as an example to detail how to implement custom post-processing effects in your project.
The core of post-processing is to process the relevant data in the frame buffer using a `ComputeShader` or `RenderShader` according to some algorithm after the scene content has been rendered, and then render it to the screen to achieve a specific rendering effect.
For this purpose, we will focus on the following points:

    1. How to create a custom post-processing object
    2. How to read the relevant data from the frame buffer
...
After creating the custom post-processing class, we need to create a `ComputeShader` object for this post-processor to process the relevant data in the frame `Buffer` and compute the output into another texture. Before creating the `ComputeShader` object, we can first create a temporary texture to store the processed data, for example:
...
In the above code:
1. A `UniformGPUBuffer` for storing the blur parameters is created using `new UniformGPUBuffer`.
2. A `ComputeShader` object is created using `new ComputeShader`, and three `binding` groups are added:
    - `args` is a `GaussianBlurArgs` type `uniform` that stores the global blur parameters.
...
3. After creating the `ComputeShader` object, the relevant data needs to be associated with it:
    ...
    - `this.mGaussianBlurShader.setSamplerTexture('colorMap', this.getLastRenderTexture())` associates the color texture attachment from the frame `Buffer` with `colorMap`. `this.getLastRenderTexture()` is a method on the base class `PostBase` used to get the texture of the previous color attachment.
...
At this point, the preparation work for the `ComputeShader` is complete.

## 3. Rendering the Processed Data
After creating the `ComputeShader` object, we still need to render the processed data to the screen, that is, the `VirtualTexture` object created at the beginning. To associate the processed data with the screen for rendering,
we need to create a `RendererPassState` using `WebGPUDescriptorCreator.createRendererPassState`:
...
`rendererPassState` is a public property on the base class `PostBase`. After associating the created `RendererPassState` object, the engine's built-in `PostRenderer` will automatically render the processed data to the screen.
...
On the first entry into `render`, since `this.mGaussianBlurShader` is not yet initialized, it will enter the `createResource` and `createComputeShader` functions to create the relevant resource objects.
In `createResource`, a virtual texture of the screen size is created to store the blurred pixel data; in `createComputeShader`, a `ComputeShader` object is created and the relevant parameters are associated.
Finally, `GPUContext.computeCommand` is used to execute the `ComputeShader`.

Since the screen window size may be adjusted, when the window size changes, the size of the blur texture needs to be recalculated and the texture object recreated to adapt to the new window size:
```ts
class GaussianBlurPost extends PostBase {
    // ...
...
## Summary
Using a Gaussian blur example, this section introduced how to create a custom post-processing effect in the engine, how to read the relevant data from the frame `Buffer` in the post-processor, and how to associate the processed data with the screen:
