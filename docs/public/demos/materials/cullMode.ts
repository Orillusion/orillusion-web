import {
    Engine3D, Vector3, Scene3D, Object3D, Camera3D, AtmosphericComponent, View3D, UnLitMaterial, MeshRenderer, OrbitController, PlaneGeometry, BitmapTexture2D, DirectLight, Color, GPUCullMode
...
async function demo() {
    ...
    let select = document.createElement('select')
        select.innerHTML = `
        <option value="none">双面渲染</option>
        <option value="back">正面渲染</option>
        <option value="front">背面渲染</option>
        `
