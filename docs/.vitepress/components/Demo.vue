<template>
    <div class="demo" :class="{
        ...
        <span class="toggle" title="Check Code" v-if="code" @click="full = !full">{{ full ? '>': '<'}}</span>
        <span class="toggle" title="Open in Codepen" v-if="codepen" @click="openCodepen" :style="{'margin-bottom': code ? '50px':''}">
